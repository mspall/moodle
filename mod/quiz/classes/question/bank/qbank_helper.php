...
class qbank_helper {
    ...
    public static function get_question_structure(int $quizid, \context_module $quizcontext,
        ...
        // If we're on any part of the question bank, we don't care if the question is a draft or not.
        $draftcheck = "AND lv.status <> :draft";
        if ($GLOBALS['PAGE']->pagetype == "question-edit" || $GLOBALS['PAGE']->pagetype == "question-bank-history-history") {
            $draftcheck = "";
        }
    ...
    /**
     * This code is used for a bugfix and is temporary.
     * The only difference between this function and the original above
     * is that this function will never check if the question is a draft.
     */
    public static function get_question_structure_bugfix(int $quizid, \context_module $quizcontext,
            int $slotid = null): array {
        global $DB;
